...
用户与认证模块负责用户登录、注销、个人信息维护、密码修改和用户管理。设备台账模块负责设备分类基础字典使用、设备入库、图片上传、列表筛选、设备详情和设备台账导出。资质审核模块负责设备证件维护、证件到期预警、审核通过和审核驳回。线上调度模块负责项目用车申请、可用设备筛选、调度排期和调度日历展示。合同管理模块负责合同草稿生成、在线预览、PDF 导出和合同扫描件上传。进场核验模块负责核验码展示、核验结果记录和业务状态推进。安全交底模块负责富文本交底、参与人签署、附件和 PDF 导出。使用监管模块负责巡检记录、固定巡检项和现场照片。故障处理模块负责故障上报、工单接受、维修处理和关闭恢复。退场评价模块负责退场申请、评分、损耗扣款、押金退还和设备状态更新。首页看板和站内消息模块负责设备统计、租赁趋势、证件预警、角色待办和通知提醒。文件访问模块负责上传文件安全校验和受控下载。
...
| 证件管理 | 管理 | 管理 | 无 | 无 | 无 | 无 |
| 资质审核 | 管理 | 审核 | 无 | 无 | 无 | 无 |
| 用车申请 | 管理 | 无 | 处理 | 提交/查看 | 无 | 查看 |
| 调度排期与调度单 | 管理 | 无 | 排期/查看 | 查看 | 无 | 查看 |
| 合同管理 | 管理/上传 | 查看 | 上传/查看 | 查看 | 无 | 查看 |
| 进场核验 | 管理 | 无 | 查看 | 核验 | 无 | 查看 |
| 安全交底 | 管理/签署 | 无 | 无 | 签署 | 创建/签署 | 无 |
| 巡检记录 | 管理 | 查看 | 无 | 查看 | 创建/查看 | 查看 |
| 故障工单 | 管理 | 受理/关闭 | 无 | 上报/查看 | 上报/查看 | 查看 |
| 退场申请 | 管理 | 查看 | 查看 | 申请 | 查看 | 查看 |
| 退场评价 | 管理 | 评价 | 查看 | 查看 | 查看 | 查看 |
| 首页看板 | 查看 | 查看 | 查看 | 查看 | 查看 | 查看 |
| 站内消息 | 查看 | 查看 | 查看 | 查看 | 查看 | 查看 |

通过权限矩阵可以看出，系统管理员拥有全局管理和完整业务演示能力，设备管理员主要负责设备入库、证件维护、资质审核、故障处理和退场评价，调度员主要负责用车申请处理、设备排期、合同生成和合同扫描件上传，项目负责人主要负责提交用车申请、进场核验、故障上报和退场申请，安全员主要负责安全交底、巡检记录和故障上报。只读审计员仅能查看其被授权的页面和记录，不能执行审核、调度、合同上传、巡检创建、故障关闭或退场评价等修改操作。该设计有助于实现业务职责隔离，降低误操作和越权操作风险。

## 4.5 业务状态流转设计

系统采用状态字段约束业务流程。设备状态包括待审核、空闲、使用中、维修中和已报废。新建设备默认进入待审核状态，审核通过后变为空闲状态，合同扫描件上传后变为使用中；故障上报可能使设备进入维修中，故障关闭后根据调度单状态恢复为使用中或空闲，退场评价后设备可变为空闲、维修中或已报废。调度单状态包括待签署、已签署、进行中、已完成和已终止，上传合同扫描件后由待签署变为已签署，进场核验通过后变为进行中。合同主流程为草稿到已签署，枚举中保留待签署状态，但当前主流程未实际写入该中间状态。故障工单状态包括待处理、处理中和已关闭。退场申请状态包括待评价和已完成。
