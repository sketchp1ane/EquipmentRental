...
获取方法一：Mermaid 转换为 SVG/PNG。正式论文中建议用 draw.io 根据该图重画为更规范的 E-R 图。

```mermaid
erDiagram
    ApplicationUser ||--o{ DispatchRequest : submits
    ApplicationUser ||--o{ AuditRecord : audits
    ApplicationUser ||--o{ DispatchOrder : dispatches
    ApplicationUser ||--o{ SafetyBriefing : creates
    ApplicationUser ||--o{ FaultReport : reports
    ApplicationUser ||--o{ ReturnApplication : applies
    ApplicationUser ||--o{ ReturnEvaluation : evaluates

    EquipmentCategory ||--o{ Equipment : contains
    Equipment ||--o{ EquipmentImage : has
    Equipment ||--o{ Qualification : has
    Equipment ||--o{ AuditRecord : has
    Equipment ||--o{ DispatchOrder : assigned
    Equipment ||--o{ InspectionRecord : inspected
    Equipment ||--o{ FaultReport : has

    DispatchRequest ||--o{ DispatchOrder : generates
    DispatchOrder ||--|| Contract : has
    DispatchOrder ||--|| EntryVerification : has
    DispatchOrder ||--o{ SafetyBriefing : has
    SafetyBriefing ||--o{ BriefingParticipant : includes
    SafetyBriefing ||--o{ BriefingAttachment : has
    DispatchOrder ||--o{ InspectionRecord : has
    DispatchOrder ||--o{ FaultReport : has
    DispatchOrder ||--|| ReturnApplication : has
    ReturnApplication ||--|| ReturnEvaluation : has
```

获取方法二：GPT-image-2 生成草图。Prompt：

```text
Create a simplified ER diagram for a Chinese undergraduate software engineering thesis. White background, simple entity boxes, thin connector lines. Put Equipment 设备 in the center and DispatchOrder 调度单 to the right. Include ApplicationUser 用户 at the top. Entities: EquipmentCategory 设备分类, EquipmentImage 设备图片, Qualification 设备证件, AuditRecord 审核记录, DispatchRequest 用车申请, Contract 合同, EntryVerification 进场核验, SafetyBriefing 安全交底, BriefingParticipant 交底参与人, BriefingAttachment 交底附件, InspectionRecord 巡检记录, FaultReport 故障工单, ReturnApplication 退场申请, ReturnEvaluation 退场评价. No attributes, only entity names. Clean academic vector style, no icons, no title.
```
...
| EquipmentCategory | 设备分类表 | Name、ParentId、Level | 与 Equipment 一对多 |
...
| Contract | 合同表 | ContractNo、Status、ScanPath | 与 DispatchOrder 一对一 |
...
| 合同 | 草稿、已签署、已终止；待签署为保留枚举，当前主流程未实际写入 |
| 安全交底 | 草稿、已完成 |
| 故障工单 | 待处理、处理中、已关闭 |
| 退场申请 | 待评价、已完成 |

其中，合同状态中的待签署对应 `ContractStatus.AwaitingSignature`，是系统预留的中间状态，当前主流程未实际写入。实际业务中，调度排期后系统生成合同草稿，调度员或管理员上传线下签署后的合同扫描件后，合同由草稿直接变为已签署，并同步推进调度单和设备状态。
