...
Equipment 表保存设备编号、设备名称、设备分类、品牌型号、所属公司和设备状态等信息，是系统最核心的数据表。EquipmentCategory 用于保存系统预置的层级设备分类基础字典，主要服务于设备入库时的分类选择、用车申请时的设备类型选择和设备列表筛选。当前系统未提供独立的设备分类 CRUD 页面，因此论文中不宜将其描述为完整的设备分类基础字典模块。EquipmentImage 保存设备图片，Qualification 保存设备证件，AuditRecord 保存资质审核记录。AuditRecords.AuditorId 表示执行资质审核动作的审核人用户 Id，是历史字段命名，不代表系统仍保留 Auditor 角色。设备审核通过后才进入空闲状态，才能参与后续调度。
...
| AuditRecord | 审核记录表 | Action、Remark、AuditedAt、AuditorId | 关联 Equipment 和 ApplicationUser；AuditorId 表示执行资质审核动作的审核人用户 Id |
