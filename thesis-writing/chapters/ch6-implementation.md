...
权限控制采用 RBAC 模型。系统将具体业务操作与角色关联，而不是直接为每个用户分配权限。设备管理员能够管理设备、证件、审核、故障和退场评价；调度员能够处理用车申请、调度排期和合同扫描件上传；项目负责人能够提交用车申请、执行进场核验和发起退场申请；安全员能够创建安全交底、巡检记录和故障上报；只读审计员只能查看被授权页面和记录，不允许执行修改类操作。通过这种设计，系统能够较好地实现职责分离和权限隔离。
...
合同管理模块支持合同在线预览和 PDF 导出。合同线下签署完成后，调度员或管理员上传合同扫描件。扫描件上传成功后，系统将合同状态推进为已签署，同步将调度单状态推进为已签署，并将设备状态置为使用中。该设计将线下签署结果和线上业务状态连接起来，为后续进场核验提供前置条件。
...
flowchart LR
    ...
    E -- 是 --> G[生成调度单和合同草稿]
    G --> H[调度员<br/>上传合同扫描件]
    H --> I[系统服务层<br/>合同已签署<br/>调度单已签署<br/>设备变为使用中]
    I --> J[项目负责人<br/>输入核验码]
...
```text
Create a clean business flowchart for a Chinese undergraduate software engineering thesis. White background, left-to-right flow, simple rounded rectangles and one decision diamond. Steps: 项目负责人提交用车申请; 系统保存申请，状态为待处理; 调度员选择可用设备并排期; 系统校验设备状态、资质有效期、时间冲突; decision 是否可调度; 否 -> 返回错误提示; 是 -> 生成调度单和合同草稿; 调度员上传合同扫描件; 合同已签署，调度单已签署，设备变为使用中; 项目负责人输入核验码; 系统校验核验码、调度单状态、设备状态和证件有效期; 生成进场核验记录，调度单变为进行中. Clean vector style, Simplified Chinese, no icons, no title.
```
...
项目负责人输入核验码后，系统服务层检查调度单是否处于已签署状态、核验码是否匹配、设备是否已处于使用中状态以及相关证件是否有效。只有调度单处于已签署状态并通过相关校验后，核验才会成功。核验通过后，系统生成进场核验记录，并将调度单状态推进为进行中。需要注意的是，设备进入使用中状态的触发点是合同扫描件上传，而不是进场核验。若核验失败，系统保存失败原因，便于后续查看和追溯。
...
使用监管模块主要包括巡检记录和固定巡检项。安全员或管理员可以针对进行中的调度单新增巡检记录，记录检查日期、整体状态、备注和现场照片。巡检项采用固定清单方式，例如外观、液压、电气、紧固件、安全装置、控制装置、作业环境和操作记录等。系统根据巡检项结果汇总整体状态，为设备使用过程监管提供结构化记录。

当巡检或现场使用过程中发现设备异常时，安全员、项目负责人或管理员可以发起故障上报，设备管理员或管理员负责故障受理、处理和关闭。故障记录包括故障描述、严重程度、现场图片、处理状态和处理结果等。通过安全交底、巡检和故障记录的结合，系统能够记录设备使用过程中的安全管理活动，为后续退场评价和责任追溯提供依据。

## 6.6 故障处理与退场评价实现

故障处理模块用于管理设备使用过程中的故障工单。安全员、项目负责人或管理员上报故障后，故障工单进入待处理状态，设备管理员可以接受并处理工单。故障处理过程中，设备状态可能由使用中变为维修中。设备管理员填写处理结果、维修费用等信息后关闭工单，系统根据业务情况恢复或更新设备状态。通过工单状态变化，系统能够记录故障从上报、处理到关闭的完整过程。
...
文件安全访问模块用于处理设备图片、证件附件、合同扫描件、安全交底附件、巡检照片和故障图片等上传文件。系统统一允许 `.jpg`、`.jpeg`、`.png` 和 `.pdf` 文件，单文件大小不超过 10 MB，并对上传文件进行扩展名白名单、MIME 类型和文件魔数校验。校验通过后，系统使用 GUID 重命名文件，降低重名覆盖、路径穿越和伪装文件风险。上传文件保存到 Uploads 目录，不直接放在 wwwroot 静态资源目录下，文件下载统一经过 FilesController 鉴权后返回。该设计符合 OWASP 对文件上传安全的基本建议[18]。
