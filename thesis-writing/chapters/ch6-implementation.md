...
项目负责人输入核验码后，系统首先根据核验码查找调度单，并检查调度单是否处于已签署状态；随后校验核验码有效期、设备状态和证件有效期。由于当前实现中合同扫描件上传后已将设备置为出租中或使用中，因此进场核验不再作为设备进入出租中状态的唯一动作。正常链路中设备已在扫描件上传后进入使用中；服务层仍保留对已签署订单下空闲状态的兼容修正。核验通过后，系统写入或更新 EntryVerification 进场核验记录，将核验结果标记为通过，并将调度单状态推进为进行中。若核验失败，系统记录失败原因，便于后续查询和追溯。
...
安全交底模块用于记录设备进场后的现场安全管理资料。安全员或系统管理员可以创建安全交底记录，填写交底日期、地点、富文本交底内容、现场参与人员名单和附件等信息。系统在保存富文本内容前使用 HtmlSanitizer 过滤危险 HTML，降低 XSS 攻击风险。安全交底附件最多 10 个，附件保存时同样经过文件服务的扩展名、MIME 类型、文件魔数和大小校验。创建交底的安全员与项目负责人可以进行系统内确认，管理员可进行代签；普通现场参与人员作为名单留存。系统记录签署时间、签署账号和客户端 IP，使安全交底过程能够被查询和追溯。该签署属于系统内业务确认与留痕，不等同于具备法律认证能力的在线电子签章。

使用监管模块主要包括巡检记录和固定巡检项。安全员或系统管理员可以针对进行中的调度单新增巡检记录，记录检查日期、整体状态、备注和现场照片。巡检照片最多 5 张。巡检项采用固定清单方式，包括外观完好性、证件齐全、防护装置、紧急制动、液压/电气系统、周边环境、操作员持证和安全标识。系统保存巡检整体状态，并同步保存 8 项明细结果，为设备使用过程监管提供结构化记录。
