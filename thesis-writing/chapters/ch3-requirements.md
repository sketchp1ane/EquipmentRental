...
系统涉及六类用户角色。不同角色拥有不同的业务职责和操作权限，系统通过基于角色的访问控制实现权限隔离。角色划分既要满足业务协同需要，也要避免职责边界过宽导致越权操作。
...
| 只读审计员 | 查看被授权的页面和记录，不执行修改类操作 | 查看设备台账、调度、核验等授权记录，不能审核、调度、上传合同、创建巡检、关闭故障或退场评价 |

通过角色划分，系统能够将设备管理、调度合同、安全监管和项目使用等职责分离，减少越权操作和职责混淆。例如，只读审计员并不是资质审核员，也不是全模块管理员，只能在系统授权范围内查看只读页面和记录；项目负责人除提交用车申请、进场核验和退场申请外，在设备使用过程中也可以根据现场情况上报故障。
...
| 安全交底 | 富文本交底、现场参与人员名单、安全员与项目负责人系统内确认、附件、PDF 导出 | 创建安全交底由安全员或系统管理员执行；管理员可代签，签署属于系统内确认留痕，不等同于法律意义电子签章 |
| 使用监管 | 巡检记录、固定巡检项、现场照片 | 巡检创建由安全员或系统管理员执行；固定巡检项为外观完好性、证件齐全、防护装置、紧急制动、液压/电气系统、周边环境、操作员持证、安全标识；巡检照片最多 5 张 |
